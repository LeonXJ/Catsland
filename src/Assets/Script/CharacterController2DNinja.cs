...
namespace Catslandx {
  ...
  public class CharacterController2DNinja : MonoBehaviour, IRelayPointCatcher, ICharacterController2D {
    ...
    private void handleGroundedMovement(Vector2 move, bool crouch, bool jump) {
      ...
      rigidbody.velocity = new Vector2(move.x * maxSpeed, rigidbody.velocity.y);
      if (jump && !crouch) {
        rigidbody.velocity = new Vector2(move.x * maxSpeed * jumpHorizontalSpeedDump, jumpForce);
      }
    }
    ...
    private void handleAirboneMovement(Vector2 move, bool jump, bool dash) {
      ...
      rigidbody.position += Vector2.right * move.x * airAdjustmentScale * Time.deltaTime;
    }
