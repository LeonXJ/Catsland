...
public class DynamicLight : MonoBehaviour {
	...
	void setLight () {
        ...
        Vector2[] polygenVertexes = getMeshVertexFromCollider(mf);
        if (polygenVertexes == null) {
          continue;
        }
				for (int i = 0; i < polygenVertexes.Length; i++) {								   // ...and for ever vertex we have of each mesh filter...
					
					verts v = new verts();
					// Convert to world space
					Vector3 worldPoint = mf.transform.TransformPoint(polygenVertexes[i]);
    ...
    Vector2[] getMeshVertexFromCollider(Collider2D collider) {
      if(collider.GetType() == typeof(BoxCollider2D)) {
        ...
        return ((PolygonCollider2D) collider).points;
      } else {
        return null;
      }
    }
